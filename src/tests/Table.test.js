...
import { act, screen, waitFor, fireEvent, waitForElementToBeRemoved } from '@testing-library/react';
...
describe('teste da rota "/carteira"', () => {
  ...
  test.only('Verifica a edição dos itens na tabela', async () => {
    ...
    const { history, store } = renderWithRouterAndRedux(<Wallet />, { initialEntries: ['/carteira'], initialState: INITIAL_STATE2 });
